...
### 4.2 Enclosure Type B: Universal Satellite Pod Housing (Pods 1 and 2)
...
The 6-pin interface is 100% identical and symmetrically wired for both satellite pods (Pod 1 and Pod 2):

| M8 / Pogo Pin | Conductor Color (PUR Cable) | Cross Section | Signal Pods 1 & 2 (Audio & Intercom) | Shielding & Twisting |
| :---: | :--- | :---: | :--- | :--- |
| **Pin 1** | **Red (RD)** | $0.34\,\text{mm}^2$ (AWG22) | **`VCC`** (5V switched power via MOSFET) | Single core (Power) |
| **Pin 2** | **Black (BK)** | $0.34\,\text{mm}^2$ (AWG22) | **`GND`** (Dedicated power & signal ground) | Single core (Power Ground) |
| **Pin 3** | **White (WH)** | $0.14\,\text{mm}^2$ (AWG26) | **`NF_P`** (Balanced Audio + via Bourns) | **Pair 1 twisted** (with Pin 4) |
| **Pin 4** | **Blue (BU)** | $0.14\,\text{mm}^2$ (AWG26) | **`NF_N`** (Balanced Audio - via Bourns) | **Pair 1 twisted** (with Pin 3) |
| **Pin 5** | **Yellow (YE)** | $0.14\,\text{mm}^2$ (AWG26) | **`TRIGGER_PPS`** (Single-Wire UART / Opto-Trigger) | Single core (Control) |
| **Pin 6** | **Green (GN)** | $0.14\,\text{mm}^2$ (AWG26) | **`1-WIRE_ID`** (Native Emulation / DS2401 ID) | Single core (1-Wire Bus) |
| **M8 Shell** | **Tinned Copper (BL)** | $> 85\,\%$ Braid | **`GND_SHIELD`** (360° Chassis Shield) | Overall shield via M8 metal body |
...
- **RF Backbone Integration (UWB):** Integrated into the enclosure bottom floor of the lower tub is a recessed antenna pocket ($11 \times 11 \times 0.6\,\text{mm}$) for the Taoglas FXUWB10 flex antenna. The ultra-short 20 mm U.FL cable routes directly to the Qorvo DW3110 transceiver on PCBA 05 `B.Cu`. This completely eliminates 2.4 GHz interference in the cockpit and provides deterministic transmission latency ($< 0.4\,\text{ms}$) to the Central Box.
- **Laminar Cockpit Sensor Hub (J12 Qwiic):** Positioned inside the aerodynamic cold air scoop of the housing is the Qwiic sensor hub with integrated u-blox SAM-M10Q Multi-GNSS patch module ($15 \times 15\,\text{mm}$ ground plane), TI TMP117 laboratory-grade temperature sensor ($\pm 0.1\,^\circ\text{C}$ black ice sentry), and TI OPT3001 ambient light sensor.
...
2. **Rear Face / North Flank ($Y = 68.0\,\text{mm}$):**
   ...
   - **U.FL UWB Antenna Outlet:** Coaxial U.FL connector on PCBA 05 leads directly to the bottom tub antenna pocket for the Taoglas FXUWB10 flex antenna, providing an ultra-low latency, deterministic wireless link to the Central Box without cables across the steering head.
3. **Right Short Flank / East Flank ($X = 98.0\,\text{mm}$):**
   ...
   - **Sensor Bus:** 4-Pin JST-SH Qwiic / Stemma QT $I^2C$ expansion port (`J12`, $Y_{\text{tub}} = 34.05\,\text{mm}$) connecting the SAM-M10Q GNSS patch module, TMP117 temperature sensor, and OPT3001 light sensor in the air scoop.
   - **Tactile Switches:** Miniature tactile switches `SW1` (Boot) and `SW2` (Reset) for MCU maintenance.
   - **Visual Status Display:** Polycarbonate light-pipe in enclosure lid for **WS2812B RGB Status LED** (`LED1`, $Y_{\text{tub}} = 40.05\,\text{mm}$): Green = Normal Operation, Blue = UWB Link Active, Yellow = USB Enumeration / CP2AA Dongle Boot, Red = CAN Error / Failsafe.
...
For classic touring and bagger models with 2-Up comfort seats or exposed rear fenders, OpenMotorBridge provides a clean, fender-free integration:
...
> [!NOTE]
> **Fender Console Retirement (Deletion of `pod3_touring_fender_console`):**  
> With the retirement of Pod 3, the rear fender on cruisers and baggers is no longer obstructed by an add-on console. The classic custom lines of the bare sheet metal fender remain 100% intact. The rear radar mounts discreetly below the license plate.

---

### 6.3 Reference Kit 3: Adventure & Touring Enduros (BMW GS / GSA, KTM Adventure, Africa Twin)
...
On exposed adventure bikes and remote expedition tracks (e.g. TET routes, alpine gravel passes, unattended fuel stops), the invisible magnetic locking mechanism safeguards swap cartridges (Sena, Cardo, OMM) against opportunistic theft:
...
Mechanically and ergonomically, the Car Support Kit relies on three screw-free, non-destructive CAD components:
1. The **Universal Sun Visor Clip for Pods 1 & 2 (2x)** ([`car_sun_visor_pod_clip.scad`](../../hardware/cad/scad/05_accessories/car_sun_visor_pod_clip.scad) / [`car_sun_visor_pod_clip.stl`](../../hardware/cad/stl/05_accessories/car_sun_visor_pod_clip.stl))
...
3. The **Compact Dashboard GNSS Dock** for the u-blox SAM-M10Q Multi-GNSS patch module near the base of the windshield.
...
#### 6.8.3 Wireless Automotive Telemetry: Autonomous Bluetooth OBD2 Coupling via Central Box
...
#### 6.8.4 Complete System Topology & Tool-Free 5-Minute Vehicle Integration
...
| **Camera Dock** | Road Glide ST Sharknose 15W Qi Inductive Cam Dock (3M Dual Lock) | `05_accessories/road_glide_inductive_cam_dock.stl` | `05_accessories/road_glide_inductive_cam_dock.scad` |
