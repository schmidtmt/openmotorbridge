...
void task_power_supervisor(void *pvParameters) {
    ESP_LOGI(TAG, "Power Supervisor Task started (KL15 & USV Monitoring)...");
    ...
    while (true) {
        ...
        if (ign_now != s_ignition_active) {
            ...
            if (s_ignition_active) {
                ...
                // Zündung AUS: Graceful Shutdown einleiten
                ESP_LOGI(TAG, "Initiating Graceful Shutdown sequence...");
                sdio_track_finalize();
                webdav_trigger_sync_sequence();
            }
        }

        // Bei Zündung AUS: Nachlaufzeit zählen
        if (!s_ignition_active) {
            ignition_off_timer_sec++;
            // Nach 90 Sekunden Nachlaufzeit oder bei schwachem USV-Akku: Deep Sleep
            if (ignition_off_timer_sec >= 90 || s_v_bat < VOLTAGE_UPS_LOW_V) {
                ESP_LOGW(TAG, "Entering Low-Power Deep Sleep mode (< 1.2 mA)...");
                // Pods abschalten
                gpio_set_level(PIN_PORT1_VCC_EN, 0);
                gpio_set_level(PIN_PORT2_VCC_EN, 0);
                
                // Aufwecken über Pegelwechsel an KL15 (GPIO 4)
        ...
        SystemTelemetry_t telem = {
            .v_ign_volts = s_v_ign,
            .v_bat_volts = s_v_bat,
            .remote_bat_pct = ble_handlebar_get_battery_level(),
...
static void on_handlebar_button_event(uint8_t button_id, bool long_press) {
    ...
    } else if (button_id == 2) { // Shutter / Actioncam Marker
...
extern "C" void app_main(void) {
    ...
    // 4. FreeRTOS Tasks starten mit strikter Core-Trennung
