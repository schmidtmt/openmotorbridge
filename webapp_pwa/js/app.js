...
function renderSystemBuilder() {
    ...
    const bikeNames = {
        'bmw-gs': 'BMW R1250 / R1300 GS (Standard / Vario)',
        'bmw-gsa': 'BMW R1250 / R1300 GSA (Adventure / Rohrträger)',
        'hd-touring': isDe ? 'Harley-Davidson Touring (Street/Road Glide, Limited, Road King)' : 'Harley-Davidson Touring (Street/Road Glide, Limited, Road King)',
    ...
    if (builderState.bike === 'bmw-gs') {
        bikeInstructions = `
            <div class="builder-instruction-step">
                <div class="builder-step-headline">
                    <span class="builder-step-name">7. ${isDe ? 'Montage an deiner BMW R1250 / R1300 GS (Standard / Vario-Koffer)' : 'Installation on your BMW R1250 / R1300 GS (Standard / Vario Panniers)'}</span>
                ...
                <div class="builder-instructions-body">
                    <ol>
                        ...
                        <li>${isDe ? '<strong>Pod 1 & 2 (Option A: Vario / Rahmenrohr):</strong> Transition-Docks (<code>adventure_transition_dock.stl</code>) in der Sitzbank-Bügelfalte an das Ø 28 mm Hauptrahmenrohr klemmen. Pod-Gehäuse verschrauben. <em>100% kofferunabhängig:</em> Baut nicht breiter als die schlanke Fahrzeug-Silhouette – fahrbar mit Vario-Koffern oder komplett ohne Koffer!' : '<strong>Pods 1 & 2 (Option A: Vario / Frame Tube):</strong> Clamp transition docks (<code>adventure_transition_dock.stl</code>) in seat crease to Ø 28 mm frame tube. <em>100% luggage-independent:</em> Does not build wider than bike silhouette – rideable with Vario cases or completely without luggage!'}</li>
                        ...
                        ${builderState.addons.frontNode ? `<li>${isDe ? '<strong>Front-Node & Cockpit (Gemeinsame Basis):</strong> Die 4x Torx T25 Schrauben des Windschilds lösen und Scheibe abnehmen. Obere TFT-Cockpitblende nach vorne ausclipsen. Front-Node mit AMPS-Halter am Ø 12 mm GPS/Navibügel fixieren. <em>100% drahtlose Funkbrücke:</em> Der Front-Knoten kommuniziert komplett kabellos über 2.4 GHz ESP-NOW (< 1,8 ms) mit der Zentralbox (kein Kabel durch den Lenkkopf!). Die 12V-Stromversorgung erfolgt direkt am originalen BMW Cartool-Navistecker (oder Standlicht KL15 & GND) im Cockpit.' : '<strong>Front Node & Cockpit (Common Base):</strong> Remove 4x Torx T25 windshield screws and lift screen off. Unclip upper TFT cover forward. Mount Front Node using AMPS pattern to Ø 12 mm GPS bar. <em>100% Wireless Link:</em> Front Node communicates completely wirelessly via 2.4 GHz ESP-NOW (< 1.8 ms) with Central Box (zero cables through steering head!). 12V power is tapped directly from OEM BMW Cartool nav connector (or parking light KL15 & GND) in cockpit.'}</li>` : ''}
                    </ol>
                </div>
            </div>
        `;
        ...
        bikeInstructions = `
            <div class="builder-instruction-step">
                <div class="builder-step-headline">
                    <span class="builder-step-name">7. ${isDe ? 'Montage an deiner BMW R1250 / R1300 GSA (Adventure / Rohrträger)' : 'Installation on your BMW R1250 / R1300 GSA (Adventure / Stainless Rack)'}</span>
                    <span class="builder-pill-verified">✓ Rohrträger-Käfig (Option B)</span>
                </div>
                <div class="builder-instructions-body">
                    <ol>
                        ...
                        <li>${isDe ? '<strong>Pod 1 & 2 (Option B: Edelstahl-Rohrkofferträger):</strong> 1,0 mm EPDM-Schutzstreifen um das Ø 18 mm Rohr wickeln. Klemmschellen (<code>adventure_pannier_rack_clamp_base.stl</code> + <code>cap.stl</code>) mit M5x30 mm V4A Schrauben und Stoppmuttern über Kreuz mit 4,5 Nm anziehen. Pod-Basisgehäuse sturzsicher im Rohrrahmen-Dreieck vor den Alukoffern verschrauben.' : '<strong>Pods 1 & 2 (Option B: Stainless Pannier Racks):</strong> Wrap 1.0 mm EPDM strip around Ø 18 mm tube. Clamp bases and caps with M5x30 mm bolts and Nyloc nuts (4.5 Nm). Bolt pod base housings crash-protected inside frame triangle ahead of aluminum cases.'}</li>
                        ...
                        ${builderState.addons.frontNode ? `<li>${isDe ? '<strong>Front-Node & Cockpit (Gemeinsame Basis):</strong> Windschild mit 4x Torx T25 demontieren, TFT-Abdeckung abnehmen. Front-Node am Ø 12 mm Bügel montieren und lokal am Cartool-Stecker mit 12V versorgen. Datenübertragung erfolgt 100% drahtlos via 2.4 GHz ESP-NOW Funkbrücke.' : '<strong>Front Node & Cockpit (Common Base):</strong> Remove screen via 4x Torx T25, unclip TFT shroud. Mount Front Node on Ø 12 mm bar and connect locally to 12V Cartool plug. Data link is 100% wireless via 2.4 GHz ESP-NOW link.'}</li>` : ''}
                    </ol>
                </div>
            </div>
        `;
        ...
        bikeInstructions = `
            <div class="builder-instruction-step">
                <div class="builder-step-headline">
                    <span class="builder-step-name">7. ${isDe ? 'Montage an deiner Harley-Davidson Touring Plattform (Street/Road Glide, Limited & Road King)' : 'Installation on your Harley-Davidson Touring Platform (Street/Road Glide, Limited & Road King)'}</span>
                    <span class="builder-pill-verified">✓ Modulare Fairing & Heck-Optionen</span>
                </div>
                <div class="builder-instructions-body">
                    <ol>
                        <li>${isDe ? '<strong>Zentralbox (Gemeinsame Basis):</strong> Unter der Fahrersitzbank auf der massiven Rahmenbrücke vor der Batterie auf 4x M4 Silentblöcken verschrauben. Die HD26-Kabelpeitsche führt nach hinten zu den Koffern und direkt zum BCM.' : '<strong>Central Box (Common Base):</strong> Mount under rider seat on frame crossmember in front of battery using 4x M4 silentblocks. HD26 harness whip branches rearward to saddlebags and BCM.'}</li>
                        <li>${isDe ? '<strong>Pod 1 & 2 (Gemeinsame Basis):</strong> Kofferdeckel-Docks (<code>saddlebag_lid_dock.stl</code>) auf den Kofferdeckeln verschrauben (M4 Senkkopf + EPDM-Dichtscheiben) oder per 3M VHB Tape befestigen. <em>(Hinweis: Street Glide, Road Glide, CVO ST, Road King und Limited nutzen baugleiche Hartschalenkoffer!)</em> M8 Kabel durch Gummitülle in den Koffer und per Trennkupplung zum Rahmen führen.' : '<strong>Pods 1 & 2 (Common Base):</strong> Mount saddlebag lid docks (<code>saddlebag_lid_dock.stl</code>) on bag lids using M4 screws + EPDM washers or 3M VHB tape. <em>(Note: Street Glide, Road Glide, CVO ST, Road King, and Limited share identical hard bags!)</em> Route M8 cables through grommets to frame disconnects.'}</li>
                        <li>${isDe ? '<strong>Heck-Pod 3 (Modulare Varianten):</strong><br>' +
                            '• <em>Standard Bagger (Street/Road Glide, Road King):</em> Organische Fender-Konsole (<code>pod3_touring_fender_console.stl</code>) flach auf Kotflügel an der 1/4"-20 Sozius-Schraube verschrauben.<br>' +
                            ...
                            '• <em>Standard Bagger (Street/Road Glide, Road King):</em> Bolt organic fender console (<code>pod3_touring_fender_console.stl</code>) flat on rear fender to 1/4"-20 seat nut.<br>' +
                        ...
                        ${builderState.addons.frontNode ? `<li>${isDe ? '<strong>Cockpit & Front-Node (Modulare Fairing-Optionen):</strong><br>' +
                            ...
                            '• <em>Option C (Headlight Nacelle - Road King / Special RK & RKS):</em><br>' +
                            '  - Keine Verkleidung! Klemmschraube des 7"-Zierrings lösen, Daymaker herausnehmen. Front-Node sitzt schwingungsentkoppelt im Hohlraum der Aluminium-Scheinwerfergondel.<br>' +
                            '  - <strong>CAN-Bus Clou Road King:</strong> Da an der Front kein Radio verbaut ist, wird der CAN-Bus <strong>direkt unter der Sitzbank am BCM-Diagnosestecker (HD26 Pins 17/18)</strong> an die Zentralbox angeschlossen! Der Front-Node in der Gondel benötigt <strong>kein CAN-Kabel</strong> an <code>J2</code> und funkt 100% drahtlos via ESP-NOW (< 1,8 ms) – null Kabelzug durch den Lenkkopf!<br>' +
                            ...
                            '• <em>Option C (Headlight Nacelle - Road King / Special RK & RKS):</em><br>' +
                            '  - No fairing! Loosen 7" trim ring pinch screw, remove Daymaker bucket. Mount Front Node inside nacelle cavity.<br>' +
                            '  - <strong>Road King CAN-Bus Architecture:</strong> Since no head unit exists in the front, CAN-bus connects <strong>directly under the seat to the Central Box via BCM diagnostic plug (HD26 pins 17/18)</strong>! The Front Node inside the nacelle needs <strong>zero CAN wiring</strong> at <code>J2</code> and communicates 100% wirelessly over ESP-NOW (< 1.8 ms) – zero wires through the steering neck!<br>' +
