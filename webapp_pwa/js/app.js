            ...
            <div class="builder-instruction-step">
                ...
                <div class="builder-instructions-body">
                    <ol>
                        ...
                        ${builderState.addons.frontNode ? `<li>${isDe ? '<strong>Front-Node & Cockpit-Fairing (BMW GS):</strong> Die 4x Torx T25 Schrauben des Windschilds lösen und Scheibe abnehmen. Obere TFT-Cockpitblende nach vorne ausclipsen. Front-Node mit AMPS-Halter am Lenker/Navibügel fixieren. <em>Drahtlose Funkbrücke:</em> Der Front-Knoten kommuniziert 100% kabellos über 2.4 GHz ESP-NOW / BLE mit der Zentralbox (kein Kabel durch den Lenkkopf!). Die 12V-Stromversorgung erfolgt direkt am originalen BMW Cartool-Navistecker (oder Standlicht) im Cockpit.' : '<strong>Front Node & Cockpit Fairing (BMW GS):</strong> Remove 4x Torx T25 windshield screws and lift screen off. Unclip upper TFT cover forward. Mount Front Node using AMPS pattern to handlebar/nav bar. <em>Wireless Link:</em> Front Node communicates 100% wirelessly via 2.4 GHz ESP-NOW / BLE with the Central Box (zero cables through the steering head!). 12V power is tapped locally from the OEM BMW Cartool nav connector or parking light.'}</li>` : ''}
                    </ol>
                </div>
            </div>
        `;
            ...
            <div class="builder-instruction-step">
                ...
                <div class="builder-instructions-body">
                    <ol>
                        ...
                        ${builderState.addons.frontNode ? `<li>${isDe ? '<strong>Front-Node & Cockpit-Fairing (BMW GSA):</strong> Windschild mit 4x Torx T25 demontieren, TFT-Abdeckung abnehmen. Front-Node montieren und lokal am Cartool-Stecker mit 12V versorgen (Datenübertragung erfolgt 100% drahtlos via 2.4 GHz Funkbrücke).' : '<strong>Front Node & Cockpit Fairing (BMW GSA):</strong> Remove screen via 4x Torx T25, unclip TFT shroud. Mount Front Node and connect locally to 12V nav plug (data link is 100% wireless via 2.4 GHz ESP-NOW).'}</li>` : ''}
                    </ol>
                </div>
            </div>
        `;
            ...
            <div class="builder-instruction-step">
                ...
                <div class="builder-instructions-body">
                    <ol>
                        ...
                        <li>${isDe ? '<strong>Heck-Pod 3:</strong> Organische Fender-Konsole (<code>pod3_touring_fender_console.stl</code>) zentrisch auf dem Heckkotflügel verschrauben. <em>(Hinweis: Bei Ultra Limited / Road Glide Limited mit werkseitigem King Tour-Pak entfällt die Fender-Konsole zugunsten einer Rohr-Klemmschellenmontage am Tour-Pak-Trägerrahmen!)</em>' : '<strong>Rear Pod 3:</strong> Center and bolt organic fender console (<code>pod3_touring_fender_console.stl</code>) to rear fender. <em>(Note: On Ultra Limited / Road Glide Limited with factory King Tour-Pak, the fender console is replaced by a tube clamp on the Tour-Pak luggage frame!)</em>'}</li>
        ...
        bikeInstructions = `
            <div class="builder-instruction-step">
                <div class="builder-step-headline">
                    ...
                    <span class="builder-pill-verified">✓ Kofferdeckel & Under-Cowl</span>
                </div>
                <div class="builder-instructions-body">
                    <ol>
                        ...
                        <li>${isDe ? '<strong>Heck-Pod 3:</strong> Aufrechtes Skeleton Dock (<code>cvo_st_undercowl_skeleton_dock.stl</code>) für Pod 3 unter der Forged-Carbon-Sitzhutze montieren (federbelastet mit vollem Abstand zu den Showa-Ausgleichsbehältern). Aerodynamische Telemetrie-Finne (<code>cvo_st_telemetry_fin.stl</code>) auf der Heck-Hutze verschrauben.' : '<strong>Rear Pod 3:</strong> Mount upright skeleton dock (<code>cvo_st_undercowl_skeleton_dock.stl</code>) for Pod 3 under forged carbon cowl (spring-preloaded, clearing Showa canisters). Bolt aerodynamic telemetry fin (<code>cvo_st_telemetry_fin.stl</code>) to rear tail cowl tab.'}</li>
                        <li>${isDe ? '<strong>Radar:</strong> Entkoppelter Kennzeichen-Radarhalter (<code>radar_license_plate_bracket.stl</code>) unter dem Kennzeichen verschrauben (CVO ST verfügt serienmäßig über das mittige Kennzeichen wie alle Touring-Modelle!).' : '<strong>Radar:</strong> Bolt decoupled license plate radar mount (<code>radar_license_plate_bracket.stl</code>) beneath license plate (CVO ST features the stock centered license plate mount identical to all Touring bikes!).'}</li>
                        ${builderState.addons.frontNode ? `<li>${isDe ? '<strong>Front-Node & Sharknose Fairing (CVO Road Glide ST):</strong> T27 Schrauben der Sharknose Innenverkleidung lösen, Fairing nach vorn abnehmen. Front-Node an der Forged-Carbon-Lenkerbrücke verschrauben und lokal an 12V anschließen (drahtlose 2.4 GHz Funkbrücke zur Zentralbox).' : '<strong>Front Node & Sharknose Fairing (CVO Road Glide ST):</strong> Remove T27 bolts on inner fairing, lift Sharknose forward. Mount Front Node on forged carbon handlebar clamp and hook up to local 12V switched power (wireless 2.4 GHz link to Central Box).'}</li>` : ''}
                    </ol>
                </div>
            </div>
        `;
