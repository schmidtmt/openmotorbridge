...
module main_box_mounting_ears(length=110.0, width=74.0, ear_len=14.0, ear_w=12.0, ear_h=5.0) {
    // 1. Bottom-Left Ear (-X, -Y corner)
    translate([0, 6.0, 0])
        rotate([0, 0, 180])
            mounting_ear_m4(ear_len=ear_len, ear_w=ear_w, ear_h=ear_h, hole_r=M4_SCREW_HOLE_R);

    // 2. Bottom-Right Ear (+X, -Y corner)
    translate([length, 6.0, 0])
        rotate([0, 0, 0])
            mounting_ear_m4(ear_len=ear_len, ear_w=ear_w, ear_h=ear_h, hole_r=M4_SCREW_HOLE_R);

    // 3. Top-Left Ear (-X, +Y corner)
    translate([0, width - 6.0, 0])
        rotate([0, 0, 180])
            mounting_ear_m4(ear_len=ear_len, ear_w=ear_w, ear_h=ear_h, hole_r=M4_SCREW_HOLE_R);

    // 4. Top-Right Ear (+X, +Y corner)
    translate([length, width - 6.0, 0])
        rotate([0, 0, 0])
            mounting_ear_m4(ear_len=ear_len, ear_w=ear_w, ear_h=ear_h, hole_r=M4_SCREW_HOLE_R);
}
