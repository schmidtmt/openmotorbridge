...
module pod_tunnel_base(length=100.0, width=60.0, height=28.0, wall=2.5, r_edge=3.0) {
    difference() {
        ...
        // 4. Transverse Cable-Tie / Zip-Tie Slots (5.0 x 2.5 mm)
        translate([25.0 - 2.5, -1.0, wall - 1.2])
            cube([5.0, width + 2.0, 2.5], center=false);
        translate([length - 25.0 - 2.5, -1.0, wall - 1.2])
            cube([5.0, width + 2.0, 2.5], center=false);

        // 5. Dual Internal Snap-Fit Catch Pockets (Rasttaschen in Seitenwänden)
        // Positioned at x = POD_BULKHEAD_X + CARTRIDGE_BASE_L - 20.0 mm
        catch_x = POD_BULKHEAD_X + CARTRIDGE_BASE_L - 21.0;
        // Left Internal Catch Pocket
        translate([catch_x, wall - 1.6, 6.0])
            cube(size=[6.0, 1.8, 12.0], center=false);

        // Right Internal Catch Pocket
        translate([catch_x, width - wall - 0.2, 6.0])
            cube(size=[6.0, 1.8, 12.0], center=false);
    }
}
