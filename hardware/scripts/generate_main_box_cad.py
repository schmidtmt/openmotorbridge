...
def render_main_box_cad(output_path):
    ...
    # 1. USB-C Waterproof Service Port & Screw-Cap on Left (at X = -22.7, Y = -37, Z = 6.5)
    draw_cylinder(ax1, -22.7, -37, 6.5, 5.5, 4.0, color='#475569', alpha=0.95, axis='y')
    draw_cylinder(ax1, -22.7, -41, 6.5, 5.0, 3.0, color='#0284c7', alpha=0.95, axis='y')
    draw_cylinder(ax1, -22.7, -39, 6.5, 5.3, 0.8, color='#ef4444', alpha=0.95, axis='y')
    
    # 2. PMMA RGB Status LED Window on Left (at X = -14.2, Y = -37, Z = 6.5)
    draw_cylinder(ax1, -14.2, -37.5, 6.5, 2.5, 1.5, color='#10b981', alpha=0.95, axis='y')
    draw_cylinder(ax1, -14.2, -38.5, 6.5, 1.8, 1.0, color='#34d399', alpha=0.95, axis='y')
    ...
    # - Left Edge Connector:
    # J5 (4-Pin JST-PH Akku + NTC at Left Edge: X_CAD = -37.7, Y_CAD = -15.0, 4.5 x 10.0 mm)
    draw_box(ax2, -40.5, -20.0, -8.9, 4.5, 10.0, 5.0, color='#ef4444', alpha=0.95, edgecolor='#dc2626', linewidth=0.6)

    # - Bottom Front Connector Rail (Y = -22 to -17):
    # Left Group:
    # J3 (USB-C Service Port at X_CAD = -22.7, Rotated 90°: 6x9 mm)
    draw_box(ax2, -25.7, -22, -8.9, 6.0, 9.0, 5.0, color='#38bdf8', alpha=0.95, edgecolor='#0284c7', linewidth=0.6)
    # J4 (3-Port RGB LED Header at X_CAD = -14.2, Vertical: 2.5x7.6 mm)
    draw_box(ax2, -15.5, -22, -8.9, 2.5, 7.6, 5.0, color='#10b981', alpha=0.95, edgecolor='#059669', linewidth=0.6)
